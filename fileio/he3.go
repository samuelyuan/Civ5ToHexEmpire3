...
func Deserialize(content []byte) *HE3Map {
	...
	fmt.Println("File version: ", version1)
	...
	fmt.Println("Map version: ", version2)
	...
	tileMap := make([][]*HE3MapTile, int(width))
	for x := 0; x < int(width); x++ {
		tileMap[x] = make([]*HE3MapTile, int(depth))
...
func ReadHE3File(filename string) [][]*HE3MapTile {
	...
	serializedData := Serialize(mapData)
	err = os.WriteFile("output_test.he3", []byte(serializedData), 0644)
	if err != nil {
